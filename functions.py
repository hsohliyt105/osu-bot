...
class HelpOsu(object):
    ...
    def pp_calc(self, mode, playdata, acc = -1, beatmap = 0): #updated on 19.03.2021
        ...
        if mode == 0:
            ...
            if count_miss > 0:
                aim_value *= 0.97 * (1 - (count_miss / total_hits) ** 0.775) ** count_miss
            ...
            if mods & helper.Mods.fl.value != 0:
                if total_hits > 500:
                    aim_value *= 1.65 + (total_hits - 500) / 1200
                elif total_hits > 200:
                    aim_value *= 1.35 + 0.3 * min(1, (total_hits - 200) / 300)
                else:
                    aim_value *= 1 + 0.35 * min(1, total_hits / 200)
        ...
        if mode == 0:
            total_value = ((aim_value ** 1.1 + speed_value ** 1.1 + acc_value ** 1.1) ** (1 / 1.1)) * multiplier
        elif mode == 2:
...
class Api(object):
    ...
    def get_food(self, school_name, local_name):
        params = {
            'KEY': EDU_API_KEY,
            'Type': "json",
            'SCHUL_NM': school_name,
            'LCTN_SC_NM': local_name
        }

        r = requests.get("https://open.neis.go.kr/hub/schoolInfo", params=params)

        try:
            school_data = r.json()['schoolInfo'][1]['row'][0]
        ...
        center_code = school_data['ATPT_OFCDC_SC_CODE']
