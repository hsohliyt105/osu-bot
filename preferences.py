...
def save_channel(channel, recent_beatmap_id = None):
    ...
    try:
        sql = f"INSERT INTO channel(id, guild_id, recent_beatmap_id) VALUES({channel.id}, {channel.guild.id}, {recent_beatmap_id})"
        cur.execute(sql)
        conn.commit()

    except:
        sql = f"UPDATE channel SET guild_id={channel.guild.id} WHERE id={channel.id}"
        cur.execute(sql)
        conn.commit()
...
def load_channel(channel_id):
    ...
    sql = f"SELECT * FROM channel WHERE id={channel_id}"
    cur.execute(sql)
    result = cur.fetchall()
